...
colors <- c(colors[1:4], colors[8:5])

swatchplot(colors)

texture <- grDevices::colorRampPalette(colors[5:8])(256)
...
mat |> 
  ...
  plot_3d(heightmap = mat, 
          ...
          solid = FALSE, #was TRUE in original code
          soliddepth = 0,
          solidcolor = lc,
          solidlinecolor = lc,
...
render_camera(phi = 38, zoom = .68, theta = 5)

rgl::rgl.snapshot('wv_pop_den_map_wvupal.png', fmt = 'png')
#rgl::rgl.postscript('wv_pop_den_map_wvupal.pdf', fmt = 'pdf')
...

{
  ...
  render_highquality(
    ...
    ground_material = rayrender::diffuse(color = "white", noisecolor = colors[1])
  )
  end_time <- Sys.time()
